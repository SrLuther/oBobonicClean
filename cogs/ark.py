...
# Canais para painéis automáticos
CHANNEL_STATUS_ARK = 1482108203453124770  # Canal de status dos servidores
CHANNEL_VINCULAR_ARK = 1480007139035582546  # Canal de vinculação Discord/Steam
CHANNEL_CONTROL_ARK = 1482356742162612285  # Canal de controle (painel com botões)
...
class ArkCog(commands.Cog, name="ARK RCON"):
    ...
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self._paineis_enviados = False  # Flag para mandar painéis apenas uma vez
        self.monitor = get_monitor()  # Inicializa monitor
        self.monitor_task = None  # Task de monitoramento contínuo
        self._monitoring_active = False

    async def cog_load(self) -> None:
        """Chamado quando o cog é carregado - envia painéis automaticamente"""
        if self._paineis_enviados:
            return
        
        self._paineis_enviados = True
        
        try:
            print("[ARK] 📡 Iniciando envio automático de painéis...")
            
            # ─── PAINEL 1: Status ARK ───
            try:
                canal_status = self.bot.get_channel(CHANNEL_STATUS_ARK)
                print(f"[ARK] Debug: Canal Status = {canal_status}")
                
                if canal_status and isinstance(canal_status, discord.TextChannel):
                    try:
                        # Limpa o canal
                        print("[ARK] 🧹 Limpando canal de status...")
                        async for msg in canal_status.history(limit=None):
                            try:
                                await msg.delete()
                            except:
                                pass
                        
                        # Envia painel com status
                        print("[ARK] 📡 Enviando painel de status dos servidores...")
                        embed = discord.Embed(
                            title="🦕 Status ARK - Servidores",
                            description="Verificando status de todos os servidores...",
                            color=discord.Color.blue()
                        )
                        msg = await canal_status.send(embed=embed)
                        await msg.pin()
                        
                        # Aguarda um pouco e depois atualiza com status real
                        await asyncio.sleep(1)
                        
                        # Adiciona todos os servidores como "verificando"
                        embed_final = discord.Embed(title="🦕 Status ARK", color=discord.Color.yellow(), timestamp=datetime.now())
                        for info in config.ARK_MAPS.values():
                            embed_final.add_field(
                                name=f"🔵 {info['name']}",
                                value="🔄 Verificando status... (use `!arkstatus` para atualizar)",
                                inline=False,
                            )
                        
                        await msg.edit(embed=embed_final)
                        print("[ARK] ✅ Painel de status enviado e fixado!")
                        
                    except Exception as e:
                        print(f"[ARK] ⚠️ Erro ao enviar painel de status: {e}")
                        import traceback
                        traceback.print_exc()
                else:
                    print(f"[ARK] ⚠️ Canal {CHANNEL_STATUS_ARK} não encontrado ou inválido")
                    
            except Exception as e:
                print(f"[ARK] ⚠️ Erro na etapa 1 (Status): {e}")
            
            # ─── PAINEL 2: Vinculação ───
            try:
                canal_vincular = self.bot.get_channel(CHANNEL_VINCULAR_ARK)
                print(f"[ARK] Debug: Canal Vincular = {canal_vincular}")
                
                if canal_vincular and isinstance(canal_vincular, discord.TextChannel):
                    try:
                        # Limpa o canal
                        print("[ARK] 🧹 Limpando canal de vinculação...")
                        async for msg in canal_vincular.history(limit=None):
                            try:
                                await msg.delete()
                            except:
                                pass
                        
                        # Envia painel de vinculação
                        print("[ARK] 🔗 Enviando painel de vinculação...")
                        embed = discord.Embed(
                        ...
                        msg = await canal_vincular.send(embed=embed, view=PainelVincularView(self.bot))
                        await msg.pin()
                        
                        # Salva no cache
                        painel = load_painel()
                        painel["channel_id"] = CHANNEL_VINCULAR_ARK
                        painel["message_id"] = msg.id
                        save_painel(painel)
                        
                        print("[ARK] ✅ Painel de vinculação enviado e fixado!")
                        
                    except Exception as e:
                        print(f"[ARK] ⚠️ Erro ao enviar painel de vinculação: {e}")
                        import traceback
                        traceback.print_exc()
                else:
                    print(f"[ARK] ⚠️ Canal {CHANNEL_VINCULAR_ARK} não encontrado ou inválido")
                    
            except Exception as e:
                print(f"[ARK] ⚠️ Erro na etapa 2 (Vinculação): {e}")
            
            # ─── PAINEL 3: Controle de Servidores ───
            try:
                canal_control = self.bot.get_channel(CHANNEL_CONTROL_ARK)
                print(f"[ARK] Debug: Canal Control = {canal_control}")
                
                if canal_control and isinstance(canal_control, discord.TextChannel):
                    try:
                        # Limpa o canal
                        print("[ARK] 🧹 Limpando canal de controle...")
                        async for msg in canal_control.history(limit=None):
                            try:
                                await msg.delete()
                            except:
                                pass
                        ...
                        print("[ARK] 🎮 Enviando painel de controle...")
                        embed = discord.Embed(
                            title="🎮 Painel de Controle ARK",
                            description="Selecione um servidor no dropdown e use os botões abaixo",
                            color=discord.Color.blurple(),
                        )
                        embed.add_field(
                            name="✨ Como Usar",
                            value=(
                                "1. 🗺️ Selecione o servidor no dropdown\n"
                        ...
                        embed.add_field(
                            name="⚙️ Operações",
                            value=(
                                "⚡ Ligar - Inicia o servidor\n"
                                "🔴 Desligar - Desliga (com confirmação)\n"
                                "🔁 Reiniciar - Reinicia (com confirmação)\n"
                                "👁️ Status - Mostra jogadores online\n"
                                "👥 Jogadores - Lista completa de jogadores"
                            ),
                            inline=False
                        )
                        embed.set_footer(text="Painel interativo - sem necessidade de comandos")
                        
                        msg = await canal_control.send(embed=embed, view=PainelControlView(self.bot))
                        await msg.pin()
                        
                        print("[ARK] ✅ Painel de controle enviado e fixado!")
                        
                    except Exception as e:
                        print(f"[ARK] ⚠️ Erro ao enviar painel de controle: {e}")
                        import traceback
                        traceback.print_exc()
                else:
                    print(f"[ARK] ⚠️ Canal {CHANNEL_CONTROL_ARK} não encontrado ou inválido")
                    
            except Exception as e:
                print(f"[ARK] ⚠️ Erro na etapa 3 (Controle): {e}")
            
            print("[ARK] 📡 Conclusão do envio automático de painéis")
        
        except Exception as e:
            print(f"[ARK] ❌ Erro crítico ao enviar painéis: {e}")
            import traceback
            traceback.print_exc()
